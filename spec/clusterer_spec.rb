...
describe Clusterer do
  context 'For identical points' do
    ...
    it 'It sets two clusters (unclustered, one for the identical points)' do
      expect DBSCAN( @input, :epsilon => 4, :min_points => 1, :distance => :euclidean_distance ).clusters.size eq 2
    end

    it 'The Cluster for unclustered points is size 0' do
      expect DBSCAN( @input, :epsilon => 4, :min_points => 1, :distance => :euclidean_distance ).clusters.size eq 0
    end
